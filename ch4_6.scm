...
(define (install-let) 
   (define (evalm exp env) (eval-helper (let->lambda exp) env))
   (put 'evalm 'let evalm))
...
;add let support in exer 4_6
(define (let->lambda exp) 
   (let  ((parameters (map car (cadr exp)))
          (body (cddr exp)))
      (let ((func (make-lambda parameters body)))
          (list 'call func (map cdr (cadr exp))))))
        ...
        (if (cond-else-clause? first)
            (if (null? rest)
                ...
                (error "ELSE clause isn't last -- COND->IF"
                       clauses))
            (if (is-cond-second-form first)  
                (make-if (cond-predicate first)
                         (list 'call (cond-function first) (cond-predicate first))
                         (expand-clauses rest))
                (make-if (cond-predicate first)
                     (sequence->exp (cond-actions first))
                     (expand-clauses rest)))))))

(define (cond-function clause)  (caddr clause))
(define (is-cond-second-form clause) (eq? (cadr clause) '=>))
