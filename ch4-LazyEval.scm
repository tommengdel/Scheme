...
(define (evalm exp env)
  (cond ((self-evaluating? exp) exp)
        ...
        ((application? exp)
         (applym (actual-value (operator exp) env)
                (operands exp)
                env))
        (else
...
(define (actual-value exp env)
  (force-it (evalm exp env)))


(define (applym procedure arguments env)
...
(define (force-it-1 obj)
  (if (thunk? obj)
      (actual-value (thunk-exp obj) (thunk-env obj))
      obj))
(define force-it force-it-2)
