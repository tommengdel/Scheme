...
(define (evalm exp env)
  (cond ((self-evaluating? exp) exp)
        ...
        ((cond? exp) (evalm (cond->if exp) env))
        ((application? exp)
...
(define (add-binding-to-frame! var val frame)
   ...
   ;(display frame))
   ; How to assign value, may be append it at last. It doesn't work!!!
   (define (last-element-pointer li) 
     (if (null? (cdr li)) 
          li
         (last-element-pointer (cdr li))))
   (set-cdr! (last-element-pointer frame) (list (cons var val))))
(define (extend-environment vars vals base-env)
 ...
 (define (lookup-variable-value var env) 
   (display "env var list")
   (display (first-frame env))
   (newline)
   (define (env-loop env) 
      (define (scan var-val-list) 
         (cond ((null? var-val-list)  (env-loop (enclosing-environment env)))
               ((eq? (caar var-val-list) var)  (cdar var-val-list))
...
(define (set-variable-value! var val env) 
   (define (env-loop env) 
      (define (scan var-val-list) 
         (cond ((null? var-val-list)  (env-loop (enclosing-environment env)))
               ((eq? (caar var-val-list) var)  (set-cdr! (car var-val-list)  val))
               (else (scan (cdr var-val-list)))))
      (if (eq? env the-empty-environment)  
          (error "Unbounded variable" var)
          (scan (first-frame env))))
...
(define (define-variable! var val env)  
   (define (scan var-val-list)
      (display (length var-val-list))
      (newline)
      (cond ((null? var-val-list)  (add-binding-to-frame! var val (first-frame env)))
            ((eq? (caar var-val-list) var)  (set-cdr! (car var-val-list) val))
            (else (scan (cdr var-val-list)))))
   (scan (first-frame env)))
