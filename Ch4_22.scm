...
(define (analyze-let exp)   
   (let  ((lambdafunc (analyze (lambda-func (let->lambda exp))))  
         (vars (let->lambdavars exp))  
         (vals (map analyze (let->lambdavals exp))))

    (lambda (env) 
        (execute-application lambdafunc (extend-environment vars (map (lambda (val)  (val env)) vals) env)))))
...
(define (analyze-variable exp) 
  ...
  (lambda (env)
     (display "var") (newline)
     (display exp) (newline)
     (display  (lookup-variable-value exp env)) (newline)
     (display "end")
     (newline)
...
(define (analyze-definition exp) 
  (let ((var (definition-variable exp))
       ...
       (lambda (env)   
          (display "Definition of:") (display var) (newline)
